...
export default function LimitCommissions() {
    document.title = "ارز تو ارز | کمیسیون معاملات اتوماتیک";
    ...
    useEffect(() => {
        setTitle("ایجاد کمیسیون معاملات اتوماتیک (Limit)");
    }, [])
    // get Limit list
    ...
    // create Limit item
    const createLimitItem = async (vals) => {
        try {
            setIsLoading(true)
        ...
        <div className="mt-4 d-flex flex-wrap justify-content-center">
            ...
            <div className="col-12 p-4 border-radius-lg card-shadow bg-white transition-height">
                <div className="mb-4 d-flex justify-content-between align-items-center">
                    <div>
                        <h1 className="is-size-5 mb-2">ایجاد کمیسیون معاملات اتوماتیک</h1>
                        <p className="is-size-7 gainsboro mb-0">
                            در این بخش شما می‌توانید کمیسیون فعلی معاملات اتوماتیک را مشاهده کنید، کمیسیون جدید ایجاد کنید و لیست کمیسیون‌های قبلی را ببینید.
                        </p>
                    </div>
                </div>
                <div className="mt-4">
                    ...
                    <Formik
                        initialValues={{ type: "LIMIT", percent: "", tax: "" }}
                        validationSchema={commissionValidator.create}
                        onSubmit={(values, actions) => {
                            createLimitItem(values)
                            actions.resetForm();
                        }}
                    >
                        {({ isValid, dirty }) => (
                ...
                >
                    {
                        limitList.length ?
                            limitList.map((item, index) => {
                                return (
                                    <tr key={index} className={currentLimit[0]?._id === item._id ? "active" : null}>
