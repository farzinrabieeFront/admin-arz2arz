...
//images
import patternImg from "../../../assets/images/auth-bg-layer.png";
import arz2arzLogo from "../../../assets/images/auth-logo.png";
import CustomizedButton from "../../form/button/Button";
import { RiUserLine } from "react-icons/ri";
import { useTheme } from "../../../context";

const AuthTemplate = ({ children, className , title}) => {
  const { theme } = useTheme();
  return (
    <div className="d-flex align-items-center ">
      <Container className="">
        <Row className={`${Styles.authTemplate} ${Styles[theme]} flex-row-reverse text-center m-auto`}>

          <Col lg={12} className="p-0">
            <div className={`${Styles.bg} center-content`}>
              <div className="elementor-shape elementor-shape-bottom" data-negative="false"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 283.5 27.8" preserveAspectRatio="none">
              ...
              <h1 className="text-white yekan-black is-size-1 z-index-99">پنل ادمین ارز تو ارز</h1>
            </div>
          </Col>
          <Col lg={12} className="pt-3" >

            <div className="col-12 text-center">
              {/* <span className={`${Styles.firstTlt} text-lightBlue yekan-black is-size-1 ml-2`}> به ادمین پنل </span> */}
              <span className={`${Styles.lastTlt} yekan-black is-size-2`}>{title}</span>
            </div>

          </Col>
          <Col lg={12} className={`${Styles.forms} ${className} pb-5`}>{children}</Col>
        </Row>
      </Container>
    </div>
  );
